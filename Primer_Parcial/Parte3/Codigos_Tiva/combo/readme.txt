...
int main(void) {
    ...
    SysCtlPeripheralEnable(SYSCTL_PERIPH_GPIOJ); // Puerto J para el Botón SW1
    ...
    bool btn_anterior = true; // Lógica pull-up (1 = sin presionar)

    while(1) {
        // --- 1. LECTURA DEL BOTÓN (CAMBIO DE MODELO) ---
        bool btn_actual = (GPIOPinRead(GPIO_PORTJ_BASE, GPIO_PIN_0) == 0);
        if(btn_actual && !btn_anterior) {
            MandarStringRasp("CAMBIO_MODO\n");
            // Freno por seguridad al cambiar
            GPIOPinWrite(GPIO_PORTH_BASE, 0x03, 0x00);
            GPIOPinWrite(GPIO_PORTM_BASE, 0x06, 0x00);
            SetMotores(0, 0);
        ...
        if(distancia > 0 && distancia <= 5 && yendo_adelante) {
            ...
            GPIOPinWrite(GPIO_PORTF_BASE, GPIO_PIN_0 | GPIO_PIN_4, 0x00);
            yendo_adelante = false;
            MandarStringRasp("OBSTACULO\n"); // Mensaje clave para que la Rasp sepa que debe girar
        }

        // --- 4. EJECUCIÓN DE COMANDOS DESDE LA RASPBERRY ---
