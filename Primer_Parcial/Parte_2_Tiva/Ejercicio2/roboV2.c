...
uint32_t MedirDistancia(void) {
    GPIOPinWrite(GPIO_PORTN_BASE, 0x20, 0x20); // N5 Trigger
    SysCtlDelay(400); 
...
int main(void) {
    ...
    // Pines de salida
    GPIOPinTypeGPIOOutput(GPIO_PORTQ_BASE, 0x01); // Buzzer
    GPIOPinTypeGPIOOutput(GPIO_PORTN_BASE, 0x20); // Trigger
    GPIOPinTypeGPIOInput(GPIO_PORTN_BASE, 0x10);  // Echo
    ...
    while(1) {
        ...
        if (velocidad_global == 0 && ticks_giro == 0) {
            ...
            // AVANCE NORMAL (LÓGICA DIRECTA)
            uint32_t ancho = (periodo * velocidad_global) / 100;
            
            // Seguridad: El ancho no puede ser igual al periodo exacto en algunos modos
